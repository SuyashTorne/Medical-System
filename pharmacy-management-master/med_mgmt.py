...
#Username : admin
#pws: admin
#if error is about win32api then write in cmd "pip install pypiwin32"
...
def again():    #for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    if flag=='apt':
        apt.destroy()
    root=Tk()
    root.title('KABIR SOFTWARE SOLUTIONS')
    Label(root,text='CHEMIST AND DRUG SHOP').grid(row=0,column=0,columnspan=5)
    Label(root,text="SCOE").grid(row=1,column=0,columnspan=5)
